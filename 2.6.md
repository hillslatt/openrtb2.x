...
| `recording` | integer | An enumeration indicating how the content was recorded: 0 = pre-recorded content, 1 = real-time/live content. |
| `airdate` | integer | UNIX timestamp indicating when the content was first broadcast or made available to the public in the geographic region where it aired (i.e. 1752057600). <br><br>Content airing for the first time at the time of the bid request should use a value of -1. <br> Content that aired prior to Jan 1, 1970 should use a value of -2. |
